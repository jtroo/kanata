...
impl SystemTray {
    ...
    where
        ...
        P: AsRef<Path>,
    {
        self.get_icon_p_impl(i.as_ref(), s.as_ref())
    }
                ...
                for p_icn in ASSET_FD {
                    ...
                    for nm in f_name {
                        ...
                        for ext in IMG_EXT {
                            ...
                            if !(nm == icn_p) {
                                icon_file.push(ext); // no icon name passed, iterate extensions
                            } else if !is_icn_ext_valid {
                                icon_file.push(ext);
                            } else {
                                trace!("skip ext");
                            } // replace invalid icon extension
                            if icon_file == blank_p {
                                continue;
                            }
                            ...
                            if !icon_file.is_file() {
                                ...
                                if p_par == blank_p
                                    ...
                                    && nm == icn_p
                                {
                                    trace!("skipping further iters {:?}", nm);
                                    continue 'p;
                                }
                            } else {
        ...
        if let Some(cfg) = CFG.get() {
            ...
            if icon_dyn.contains_key(&path_cur_cc) {
                if let Some(icon) = icon_dyn.get(&path_cur_cc).unwrap() {
                    ...
                    info!(
                        "this config has no associated icon, using default: {}",
                ...
                if let Some(ico_p) = &self.get_icon_p(&cfg_icon_p, &path_cur_cc) {
                        ...
                        .build(&mut temp_icon_bitmap)
                    {
                        ...
                        warn!(
                            "✗ Invalid/no icon from this config: {}",
                            path_cur_cc.display().to_string()
                        );
                        self.tray.set_icon(&self.icon);
                    }
                    ...
                    warn!(
                        "✗ Invalid icon path from this config: {}",
                        path_cur_cc.display().to_string()
                    );
                    self.tray.set_icon(&self.icon);
                }
                        ...
                        if i == 0 {
                            ...
                            if let Some(ico_p) = &d.get_icon_p(&cfg_icon_p, &cfg_p) {
                                    ...
                                    .build(&mut temp_icon_bitmap)
                                {
                                    ...
                                    info!(
                                        "✗ main 0 icon ✓ icon path, using DEFAULT icon for {:?}",
                                        cfg_p
                                    );
                                }
                            } else {
