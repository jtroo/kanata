...
#[no_mangle] pub extern "win64" fn input_ev_listener(vk:c_uint, sc:c_uint, up:c_int) -> LRESULT {
  ...
  if let Some(mut fnhook) = h_cbl.take() { // move our opt+boxed closure, replacing it with None, can't just .unwrap since Copy trait not implemented for dyn fnMut
    ...
    *h_cbl = Some(fnhook); // put our closure back
    if handled { // now try to get the out key events that another thread should've sent via
      #[cfg(    feature="perf_logging") ]debug!(" 🕐{}μs   →→→✓ {key_event} from {vk} sc={sc} up={up}",(start.elapsed()).as_micros());
      #[cfg(not(feature="perf_logging"))]debug!(       "   →→→✓ {key_event} from {vk} sc={sc} up={up}");
      1} else {0}
  } else {error!("fnHook processing key events isn't available yet {key_event} from {vk} sc={sc} up={up}"); 0}
}
