...
When the environment variable defined by name
has the corresponding value when running kanata,
the aliases within will be active.
Otherwise, the aliases will be skipped.

A use case for `defaliasenvcond` is if one has multiple devices
which vary in the number and layout of keys,
