...
All of these actions accept either three or five number strings, depending on the
value of the first number. The first number is determines if acceleration is on 
(1) or off (0). 

If acceleration is on, then the action will accept four more strings. The 
second number is the interval (unit: ms) between movement actions. The third 
number is the time it takes (unit: ms) to linearly ramp up from the minimum 
distance to the maximum distance. The fourth and fifth numbers are the minimum 
and maximum distances (unit: pixels) of each movement.

If acceleration is off, then the action will accept two more strings. The 
second is the interval (unit: ms) between movement actions and the third number 
...
(defalias
  ...
  ms↑ (movemouse-up 0 1 1)
  ms← (movemouse-left 0 1 1)
  ms↓ (movemouse-down 0 1 1)
  ms→ (movemouse-right 0 1 1)

  ma↑ (movemouse-up 1 1 1000 1 5)
  ma← (movemouse-left 1 1 1000 1 5)
  ma↓ (movemouse-down 1 1 1000 1 5)
  ma→ (movemouse-right 1 1 1000 1 5)
)
