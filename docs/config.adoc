...
[cols="1,2"]
|===
| `$keyN`: The name of a key.
| This can be a default key name or one defined in <<deflocalkeys>>.
|===
...
For example, excluded keys can activate while a `tap-hold` decision is still pending,
...
This is name used to reference this layer in other actions.
...
will give the correct number in the `num: <number>` section.
Between the default and Interception versions, some
...
- antecedent `+tap-hold-press+` actions will not trigger their early hold
- antecedent `+one-shot+` actions will not be released
- `fork|switch` logic will not see the key

This option is disabled by default.
The reason this is not enabled by default
is because some keys may not work correctly if they are processed by Kanata.
