...
Requires a `defhands` directive. Supports keyword options for fine-grained control.
...
optional keyword pairs. It requires `defhands` to be defined.
...
| `:timeout` | `tap`, `hold` | `tap` | Action when hold timeout elapses (optimized for home-row-mod behavior).
| `:same-hand` | `tap`, `hold`, `ignore` | `tap` | Action when a same-hand key is pressed.
| `:neutral-keys` | `(key list)` | (empty) | Keys treated as neutral, overriding their `defhands` assignment.
| `:neutral` | `tap`, `hold`, `ignore` | `ignore` | Action when a `:neutral-keys` key is pressed.
| `:unknown-hand` | `tap`, `hold`, `ignore` | `ignore` | Action when a key not in `defhands` is pressed.
|===
