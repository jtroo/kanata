...
You can release a held key or layer via the related actions:

* `release-key`: release a key, as per `defsrc` compatible names
* `release-layer`: release a while-held layer

An example practical use case for `release-key` is seen in the `multi` section
directly below. There is currently no known practical use case for
...
This activates an action until either the timeout expires or a different key is
pressed. The `+one-shot+` action must be followed by a timeout (unit:
milliseconds) and another key or action.
