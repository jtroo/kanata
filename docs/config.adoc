...
actions are explained further on in this document. The whitespace story is the
same as with `+defsrc+`. The order of keys/actions in `+deflayer+` corresponds
to the physical key in the same sequence position defined in `+defsrc+`.
...
using special actions. There is currently a maximum of 25 layers allowed.
...
Without this, the special actions `+rpt+`, `+tap-hold-release+`, and
...
see the [windows-altgr](#windows-only-windows-altgr) option below.
...
This option can be used to enable the `cmd` special action in your
configuration. The `+cmd+` action allows kanata to execute programs with
...
kanata to execute arbitrary binaries even if you're testing out a configuration
...
aliases. Using the `defalias` configuration entry, you can introduce a shortcut
label for an action.
...
section explains the available special actions.
...
The action `+rpt+` repeats the most recently pressed key. Holding down this key
...
will do nothing. This might be desirable on an incomplete layer in place of a
transparent key so that a key that is not intended to be mapped will do nothing
as opposed to typing a letter.
...
You use unicode character as the alias itself if desired.

NOTE: the unicode action may not be correctly accepted by the targeted
application.
...
You may want to remap a key to automatically be pressed alongside modifiers
such as Control or Shift. There is a shortcut for this: prefix the normal key
name with one or more of:
...
The `+tap-dance+` special action allows repeated tapping of a key to result in
different actions. It is followed by a timeout (unit: milliseconds) and a list
...
You may put normal keys or other special actions in `+tap-dance+`.
...
The `+one-shot+` special action is similar to "sticky keys", if you know what
that is. This activates an action until either the timeout expires or a
different key is pressed. The `+one-shot+` special action must be followed by a
timeout (unit: milliseconds) and another key or action.

Some of the intended use cases are pressing a modifier for exactly one key or
...
Pressing multiple `+one-shot+` keys in a row within the timeouts will combine
the actions of those keys and reset the timeout to the latest that of the most
