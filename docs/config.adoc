...
optional option lists in the form `(name value)`. It requires `defhands` to be defined.
...
| `(neutral-keys (key list))` | `(key list)` | (empty) | Keys treated as neutral, overriding their `defhands` assignment.
