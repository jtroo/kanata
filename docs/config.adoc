...
xmodmap/xkb, the "U" that kanata outputs may not become a "U" after the
userspace remapping. You can use `evtest` against kanata and press the key that
outputs "U" to see which underlying key code is sent. Then you can use this
configuration to change the behaviour.
