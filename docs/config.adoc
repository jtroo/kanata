...
ifdef::env-github[]
* https://github.com/jtroo/kanata/blob/v1.12.0/docs/config.adoc[v1.12.0]
...
ifndef::env-github[]
* link:/config-1.12.0.html[v1.12.0]
...
This has consequences for conditional content, e.g. with `if-equal`.
...
NOTE: On Linux the https://github.com/tompassarelli/glide[Glide project]
can integrate touchpad gestures with Kanata.
...
| Enable physical key event notifications for this TCP connection. Set `enabled` to `false` to stop them. The server responds with `{"status":"Ok"}`.
|===

Physical key notifications are disabled for each new connection until it subscribes. This avoids
exposing high-volume, privacy-sensitive typing data to TCP clients that do not need it.
...
| Sent before remapping for a subscribed connection. `key` is the physical key name, `code` is Kanata's platform-specific numeric OS code, and `state` is `press`, `release`, `repeat`, or `tap`.
|===

`PhysicalKey` reports configured physical input in Kanata's processing order, including modifier
reordering. It does not report fake keys, macros, or generated output. Internal wake-up events are
also excluded. Because this stream can reveal everything typed, bind the TCP server only to a
trusted interface and subscribe only trusted clients.
...
| Response to `Hello`. Contains server version, protocol version, and supported capabilities. Includes `hold-activated`, `tap-activated`, and `physical-key-events`.
