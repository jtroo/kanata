...
by using the `--watch` command line flag. This requires building kanata with the
...
* The `--watch` flag is only available when kanata is compiled with the `watch` feature
