...
(tap-hold-release-tap-keys-release $tap-repress-timeout $hold-timeout $tap-action $hold-action $tap-keys)
...
This variant behaves nearly identically to `tap-hold-release-keys`
except that the early tap activates on a press->release
of a listed key, as opposed to only on a press.
...
(defalias
  ...
  umk (tap-hold-release-tap-keys-release 200 200 u @msc (a o e))
)
----
