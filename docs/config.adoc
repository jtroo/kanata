...
|`$chordN`
| A set of characters.
The order is not important; this represents all input keys being
pressed simultaneously according to kanata.
You can use space by including it as the first character in the chord.
For example, ` ap`.
When there are 0 optional follow chords,
the corresponding output on the same line `$outputN` will activate.
...
If using a space in the follow chord, use two spaces; e.g. ` ap  bc`.
