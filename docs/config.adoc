...
as well as for `layer-while-held`,
...
For all other action categories,
the release behaviour is a little more confusing:
...
It will probably be difficult to know which key was pressed first.
In these scenarios, to keep the behaviour from being confusing,
you should hold down all keys if you want to keep holding,
and release all keys if you want to do a release.

If you want to ignore the behaviour of keys being held
as described in the first paragraph of this section,
you could change the input chord's action to be a macro instead,
which will guarantee a rapid press+release of all keys being outputted.
