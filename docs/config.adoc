...
strings are arguments to that program, which are provided to the program in the
...
* `+press+`: Press the fake key which will not be released until a release or tap.
* `+release+`: Release the fake key. If it's already released, this does nothing.
