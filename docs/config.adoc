...
| `rpt-exclude-bspace`
| String action that outputs the single most-recently typed key, ignoring backspace.
|===
...
The `rpt-exclude-bspace` action repeats the last key output that isn't backspace. Note that this does not output chords, so `ctrl+c bspace` will output `c`.

.Example:
[source]
---
(deflayer has-repeat-skip-bspace
  rpt-skip-bspace a s d f
)
---
