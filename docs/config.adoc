...
The `caps-word` state ends when there are no active keys
within the timeout period, or a terminating key is pressed.
Terminating keys are every key
...
if you want to define which keys are capitalized (2nd parameter)
