...
Optionally surrounded within quotes.
Examples: `backspace`, `"string with spaces and 1 number"`.

| list
| A sequence of strings or nested lists within parentheses.
List items are separated by any amount of whitespace characters,
or by parentheses.
Examples: `(lrld-num 1)`, `(tap-dance 200 (f1(unicode 😀)f2(unicode 🙂)))`.
Unlike the second example above,
it is encouraged to have spaces after closing parentheses
unless they are immediately followed by more closing parentheses
