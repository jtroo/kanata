...
Instead of specifying an input key, you can use either `_`, `__`, or `___` to map all
the keys that are not explicitly mapped in the layer (like `caps` in the example above). 

* `_` maps only keys that are in defsrc.
* `__` excludes mapping keys that are in defsrc.
* `___` maps both, keys that are in `defsrc`, and that are not.
