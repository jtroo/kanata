...
impl Default for WinMsgSid {
    fn default() -> Self {
        Self("kanata_4117d2917ccb4678a7a8c71a5ff898ed".to_string())
    } //TODO: replace with str
}
impl From<WinMsgSid> for String {
    fn from(val: WinMsgSid) -> Self {
        val.0
    }
}
impl Deref for WinMsgSid {
    type Target = String;
...
where
    T: ?Sized,
    <WinMsgSid as Deref>::Target: AsRef<T>,
{
    fn as_ref(&self) -> &T {
        self.deref().as_ref()
    }
    ...
    if !ac_params.is_empty() {
        ...
        if let Some(a) = arg.atom(s.vars()) {
            argu = match str::parse::<usize>(a.trim_atom_quotes()) {
                ...
                Err(_) => bail_expr!(
                    &arg,
                    "invalid numeric argument, expected {}–{}. {} {}",
                    usize::MIN.to_formatted_string(&Locale::en).blue(),
                    usize::MAX.to_formatted_string(&Locale::en).blue(),
                    cmd_name,
                    ERR_MSG
                ),
            }
        ...
        if let Some(a) = arg.atom(s.vars()) {
            argi = match str::parse::<isize>(a.trim_atom_quotes()) {
                ...
                Err(_) => bail_expr!(
                    &arg,
                    "invalid numeric argument, expected {}–{}. {} {}",
                    isize::MIN.to_formatted_string(&Locale::en).blue(),
                    isize::MAX.to_formatted_string(&Locale::en).blue(),
                    cmd_name,
                    ERR_MSG
                ),
            }
