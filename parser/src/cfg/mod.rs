...
fn parse_defhands(expr: &[SExpr], s: &ParserState) -> Result<custom_tap_hold::HandMap> {
...
fn parse_tap_hold_opposite_hand(
    ac_params: &[SExpr],
    s: &ParserState,
) -> Result<&'static KanataAction> {
    ...
    let mut i = 3;
    while i < ac_params.len() {
        let kw = ac_params[i].atom(s.vars()).ok_or_else(|| {
            anyhow_expr!(&ac_params[i], "expected keyword option starting with ':'")
        })?;
        ...
        if i >= ac_params.len() {
            bail_expr!(
                &ac_params[i - 1],
                "option '{}' requires a value after it",
                kw
            );
        }
        match kw {
            ":timeout" => {
                timeout_behavior = parse_decision_behavior_tap_hold(&ac_params[i], s)?;
            }
            ":same-hand" => {
                same_hand = parse_decision_behavior(&ac_params[i], s)?;
            }
            ":neutral" => {
                neutral_behavior = parse_decision_behavior(&ac_params[i], s)?;
            }
            ":unknown-hand" => {
                unknown_hand = parse_decision_behavior(&ac_params[i], s)?;
            }
            ":neutral-keys" => {
                neutral_keys = parse_key_list(&ac_params[i], s, "neutral-keys")?;
            }
            _ => bail_expr!(
                &ac_params[i - 1],
