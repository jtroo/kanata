...
// so that the key slice is more "cheaply" cloneable
// and addressable.
// In practice
#[derive(Debug, Clone)]
...
where
    K: Clone + PartialEq + Ord + Hash + Send,
    V: Clone + Send,
{
    fn default() -> Self {
        Self::ssm_new()
    ...
    V: Clone,
{
    ...
    pub fn ssm_insert_ksorted(
        ...
        let kv = SsmKeyValue::ssmkv_new(key.as_ref(), val.clone());
        for k in key.as_ref().iter().cloned() {
                ...
                .binary_search_by(|probe| probe.key.as_ref().cmp(key.as_ref()))
            {
                Ok(pos) => {
                    ...
                    keyvals_for_key_item[pos] = kv.clone();
                }
                Err(pos) => {
                    keyvals_for_key_item.insert(pos, kv.clone());
                }
            }
        }
        key_existed
